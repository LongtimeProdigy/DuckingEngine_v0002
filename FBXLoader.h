...
public:
	...
	const bool CreateSubMesh(FbxMesh* mesh, FbxNode* node, float3& translation, float3& rotation, float3& scale, _OUT_ Model* outModel) const;

	const bool LoadFBXSkeletonFromFile(_IN_ const char* path, _IN_ const ModelRef& model, _OUT_ Skeleton& outSkeleton);
	const bool LoadFBXAnimationFromFile(_IN_ const char* path, _OUT_ Animation& outAnimation);

//#pragma region Assimp
//#ifdef USE_ASSIMP
//	const bool LoadFBXMeshFromFileByAssimp(_IN_ const char* path, _OUT_ Model* outModel) const;
//	void InitMeshFromAssimpToSubMesh(_IN_ const aiMesh* assimpMesh, _OUT_ std::vector<Vertex>& vertices, _OUT_ std::vector<uint>& indices) const;
