...
void main(uint3 id : SV_DispatchThreadID)
{
    ...
    *
    *   여기서 복소수(벡터)는 하나의 정현파를 나타내는데, 그 개념을 잘 이해 해야함
    *   복소평변에서 복소수가 회전할 때 허수값은 sin잔동을 띄며 실수값은 cos진동을 한다고 생각해야함
    *   그리고 두 sin, cos을 더하는 것이 하나의 정현파로 이는 하나의 축(허수 또는 실수)하나의 값이 파도의 높이를 의미하지 않는다는 뜻
    *   결국 복소수가 가지는 실수/허수 값 모두가 파도의 에너지이며 곧 파도의 높이기 때문에 어느 한쪽을 버리고 값을 취하면 안됨
    *   하나의 값만 취하게 되면 위에서 0.5를 곱했기 때문에 에너지의 보존이 아니게되니 매우 주의하길 바란다
    */
